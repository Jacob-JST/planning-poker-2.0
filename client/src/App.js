...
function App() {
  ...
  const [selectedSessionSummary, setSelectedSessionSummary] = useState(null); // Track selected summary
  ...
  const theme = createTheme({
    palette: {
      ...
      background: { default: darkMode ? '#303030' : '#f5f5f5', paper: darkMode ? '#424242' : '#fff' },
    },
  ...
  useEffect(() => {
      ...
      if (currentSession) {
        setSessions(prev => {
          ...
          if (existingIndex !== -1) {
            ...
            setSelectedSessionSummary(newSessions[existingIndex]); // Set as selected for display
          }
          return newSessions;
        });
        setScreen('summary');
    ...
    socket.on('connect_error', (error) => {
      ...
      alert(`Failed to connect to the server: ${error.message}. Please ensure the server is running on http://localhost:3001.`);
    });
  ...
  const handleReturnToLobby = () => {
    setScreen('home');
    setCurrentSession(null);
    setSelectedSessionSummary(null); // Clear selected summary
  };
  ...
  return (
    <ThemeProvider theme={theme}>
      <CssBaseline />
      {screen === 'login' && <Login onLogin={handleLogin} darkMode={darkMode} onDarkModeToggle={handleDarkModeToggle} />}
      {screen === 'home' && (
        <Home
          myName={myName}
          socket={socket}
      ...
      {screen === 'summary' && (
        <Summary
          summary={selectedSessionSummary ? selectedSessionSummary.results : (sessions.length > 0 ? sessions[sessions.length - 1].results : [])}
          sessionName={selectedSessionSummary ? selectedSessionSummary.sessionName : (sessions.length > 0 ? sessions[sessions.length - 1].sessionName : '')}
          isAdmin={isAdmin}
          socket={socket}
          darkMode={darkMode}
          onDarkModeToggle={handleDarkModeToggle}
      ...
      {screen === 'farewell' && <Farewell darkMode={darkMode} onDarkModeToggle={handleDarkModeToggle} />}
    </ThemeProvider>
  );
}
