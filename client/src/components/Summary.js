...
function Summary({ summary, sessionName, isAdmin, socket, darkMode, onDarkModeToggle, onLogout, onReturnToLobby }) {
  ...
  summary.forEach(s => {
    ...
    if (!aggregatedSummary[key]) {
      aggregatedSummary[key] = { votes: {}, finalEstimate: s.story.finalEstimate };
    }
    s.votes.forEach(v => {
  ...
  return (
    <Box sx={{ p: 4, textAlign: 'center', position: 'relative' }}>
      ...
      <Typography variant="h4" gutterBottom>Session Summary: {sessionName}</Typography>
      {cards.length > 0 ? (
            ...
            <Grid item xs={12} sm={4} key={index}>
              ...
              >
                <Box sx={{ textAlign: 'left' }}>
                  ...
                  <Typography variant="body2" sx={{ mb: 1 }}>{card.summaryText}</Typography>
                  <Typography variant="subtitle1" fontWeight="bold">Description</Typography>
                  <Typography variant="body2" sx={{ mb: 1, whiteSpace: 'pre-wrap' }}>{card.description}</Typography>
                  <Typography variant="subtitle1" fontWeight="bold">Votes</Typography>
                  <Typography variant="body2">{card.votesList}</Typography>
                </Box>
                <Box sx={{ textAlign: 'left' }}>
                  <Typography variant="subtitle1" fontWeight="bold">Final Estimate</Typography>
                  <Typography variant="body2">{card.finalEstimate || 'Not set'}</Typography>
                </Box>
              </Box>
            </Grid>
          ))}
      ...
      <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center', gap: 2 }}>
        <Button variant="contained" onClick={onReturnToLobby}>Return to Lobby</Button>
